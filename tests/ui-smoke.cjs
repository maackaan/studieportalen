...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.app-version').textContent(), 'Version 0.1.0-beta.2');
    ...
    await page.locator('#addForm button[type=submit]').click();
    await page.waitForFunction(() => JSON.parse(localStorage.getItem('studieportalen-data-v2')).resources.length === 2);
    ...
    await page.waitForFunction(() => JSON.parse(localStorage.getItem('studieportalen-data-v2')).events.length === 7);
    await importCalendar();
    await page.locator('.calendar-choices input').first().waitFor();
    ...
    assert.equal(await page.evaluate(() => state.events.length),7);
    await page.getByRole('button',{name:'Ångra',exact:true}).click();
    assert.equal(await page.evaluate(() => state.events.length),8);
    ...
    assert.equal(await page.evaluate(async () => (await getFile(state.resources.find(x=>x.fileName).id)).text()),'lokalt filinnehåll');
    ...
    assert.equal(JSON.parse(backup).files.length,1);
    await page.reload();
    assert.equal(await page.locator('#courseCount').textContent(),'1');
    await page.evaluate(() => deleteResource(state.resources.find(x=>x.fileName).id));
    ...
    await page.evaluate(async () => {
      const id = state.resources.find(x=>x.fileName).id;
    ...
    await page.locator('[data-close-preview]').click();
    await page.evaluate(async () => { await storeFile(state.resources.find(x=>x.fileName).id,new File(['lokalt filinnehåll'],'test.txt',{type:'text/plain'})); });
