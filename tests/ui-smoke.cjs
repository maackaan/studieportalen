...
(async () => {
  ...
  try {
    ...
    await page.locator('#dateValue').dispatchEvent('change');
    assert.equal(await page.locator('#dateEndValue').inputValue(), '2026-12-15T12:00');
    await page.locator('#addForm button[type=submit]').click();
    ...
    for (const width of [1280,768,360]) {
      ...
      assert.ok(await page.locator('.calendar-week-body').evaluate(element => element.getBoundingClientRect().height >= 1100),`Calendar grid collapsed at ${width}px`);
