...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('#dateDuration').inputValue(), '60');
    await page.locator('#dateType').selectOption('exam');
    assert.equal(await page.locator('#dateDuration').inputValue(), '240');
    assert.ok(await page.locator('#dateDuration option[value="300"]').count());
    await page.locator('#dateType').selectOption('lecture');
    assert.equal(await page.locator('#dateDuration').inputValue(), '120');
    ...
    await page.evaluate(() => { calendarWeekStart = startOfWeek(new Date(2026, 11, 15)); renderCalendar(); });
    assert.match(await page.locator('#calendarWeek').textContent(), /Allmän studiedag/);
    assert.equal(await page.locator('#calendarResultCount').textContent(), '1 post');
    assert.equal((await page.locator('.calendar-time-axis span').first().textContent()).trim(), '08:00');
    await page.locator('.calendar-event').click();
    assert.equal(await page.locator('#eventPreviewDialog').evaluate(dialog => dialog.open), true);
    ...
    await page.locator('#eventPreviewToggle').click();
    assert.equal(await page.evaluate(() => state.events[0].completed), false);
    await page.locator('#eventPreviewEdit').click();
    assert.equal(await page.locator('#dateDuration').inputValue(), '120');
    await page.getByRole('button',{name:'Avbryt',exact:true}).click();
    const decemberWeek = await page.locator('#calendarWeekLabel').textContent();
    await page.locator('#calendarNextWeek').click();
    assert.notEqual(await page.locator('#calendarWeekLabel').textContent(), decemberWeek);
    await page.locator('#calendarPreviousWeek').click();
    assert.equal(await page.locator('#calendarWeekLabel').textContent(), decemberWeek);
    await page.locator('#calendarToday').click();
    assert.equal(await page.locator('.calendar-day-heading.today').count(), 1);
    ...
    await page.locator('#courseScheduleUrl').fill('https://cloud.timeedit.net/liu/web/schema/ri1Q7.html');
    await page.locator('#addForm button[type=submit]').click();
    ...
    await page.locator('[data-copy-course-code]').click();
    await page.waitForFunction(() => document.querySelector('#toast')?.textContent.includes('TEST01 har kopierats'));
    assert.match(await page.locator('#toast').textContent(), /TEST01 har kopierats/);
    ...
    assert.ok(await page.evaluate(() => state.events.find(event => event.name === 'Tentamen 2').endDate));
    ...
    for (const width of [1280,768,520,360]) {
      ...
      assert.ok(await page.locator('.calendar-event').count() >= 5,`Calendar blocks are missing at ${width}px`);
      assert.ok(await page.locator('.calendar-week-body').evaluate(element => element.getBoundingClientRect().height >= 950),`Calendar grid collapsed at ${width}px`);
    ...
    assert.ok(data.events.find(event => event.name === 'Allmän studiedag').endDate);
    assert.equal(await page.evaluate(async () => (await getFile(state.resources.find(x=>x.name === 'Testfil').id)).text()),'lokalt filinnehåll');
